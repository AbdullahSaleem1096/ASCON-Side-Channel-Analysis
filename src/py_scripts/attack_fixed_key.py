...
# 3. Split Dataset (5000 profiling, 1000 attack)
num_profiling = 5000
...
print(f"Fixed-Key Attack Success Rate (Accuracy): {accuracy * 100:.2f}%")

# Save model
model_save_path = os.path.join(MODELS_DIR, 'model_fixed_key.h5')
...
plt.title('Loss over Epochs (Fixed Key)')
plt.legend()

plt.subplot(1, 2, 2)
plt.plot(history.history['accuracy'], label='Train Accuracy')
plt.plot(history.history['val_accuracy'], label='Val Accuracy')
plt.title('Accuracy over Epochs (Fixed Key)')
plt.legend()
plot_save_path = os.path.join(OUTPUT_DIR, 'fixed_key_history.png')
